...
class GuardCase(unittest.TestCase):
    ...
    def assertPasses(self, payload, cwd=None, msg=""):
        r = run(payload, cwd=str(cwd or self.project))
        self.assertNotEqual(
            r.returncode, BLOCKED, f"막으면 안 되는데 막힘: {msg or payload}\n{r.stderr}"
        )
...
class ProtectedPaths(GuardCase):
        ...
        self.assertBlocked(
            {
                ...
                "tool_input": {"notebook_path": "assessment/data/nb.ipynb"},
            }
        )
        self.assertBlocked(
            {"tool_name": "NotebookEdit", "tool_input": {"notebook_path": "/x/.env.local"}},
            cwd=self.bare,
        )
    ...
    def test_보호_경로_읽기는_통과한다(self):
        ...
        self.assertPasses(
            bash("sed 's/x/y/' assessment/data/cases/x.json"), msg="-i 없는 sed 는 읽기"
        )
    ...
    def test_CLAUDE_PROJECT_DIR_가_있으면_그걸_먼저_본다(self):
        r = run(
            bash("git add assessment/data/x"),
            cwd=str(self.bare),
...
class SecretsBlocked(GuardCase):
    ...
    def test_명령_치환으로_숨긴_읽기는_막는다(self):
        ...
        self.assertBlocked(bash("rg x $(cat .env.local)"), cwd=self.bare)
        self.assertBlocked(
            bash("python -c \"print('$(cat .env.local)')\""),
            cwd=self.bare,
    ...
    def test_점_없는_credentials_도_비밀값이다(self):
        ...
        self.assertBlocked(
            tool("Read", "/home/x/.config/gcloud/application_default_credentials.json"),
            cwd=self.bare,
        )
...
class SecretsAllowed(GuardCase):
    ...
    def test_존재와_추적_여부_확인은_통과한다(self):
        self.assertPasses(
            bash(
                'ls -la .env.local 2>/dev/null && echo "--- 추적 여부 ---" '
                '&& git ls-files .env.local && echo "(추적됨!)" || echo "(git 에 없음)"; '
                "git check-ignore -v .env.local 2>/dev/null; "
                'echo "=== 히스토리에 있었나 ==="; git log --oneline --all -- .env.local | head -3'
            ),
            cwd=self.bare,
        )
        self.assertPasses(
            bash("ls -la .env $H/.env ~/x/.env 2>/dev/null | awk '{print $NF, $5}'"), cwd=self.bare
        )
        ...
        self.assertPasses(
            bash("stat -c %a .env.local; wc -l .env.local; chmod 600 .env.local"), cwd=self.bare
        )

    def test_변수로_존재만_확인하는_것은_통과한다(self):
        self.assertPasses(
            bash('for f in .env $H/.env; do [ -f $f ] && echo "$f 있음"; done'), cwd=self.bare
        )
    ...
    def test_환경에_올려_쓰는_것은_통과한다(self):
        self.assertPasses(
            bash(
                ...
                'AIHUB_APIKEY="${AIHUBSHELL_API_KEY:-}" bash ~/aihub_data/fetch_samples.sh 2>&1 '
                '| grep -vE "^\\s*$" | tail -25'
            ),
            cwd=self.bare,
        )
        ...
        self.assertPasses(
            bash(
                "set -a; . .env.local; set +a; python -c 'import os; print(len(os.environ[\"K\"]))'"
            ),
            cwd=self.bare,
            msg="set +a 는 맨 set 이 아니다",
        )
    ...
    def test_비밀값_파일을_찾는_검색은_통과한다(self):
        for cmd in (
            ...
            "| grep -E 'assessment/data|\\.key$|credentials|secret' || echo ok",
            'grep -nE "ai-agent-dotfiles|local/\\.env|set -a" ~/.bashrc 2>/dev/null | head -5',
            'grep -n "def load_env_file" -A 25 src/backend.py '
            '| grep -n "Path\\|\\.env\\|environ" | head',
            'echo "데이터·비밀값 경로: '
            "$(git diff --name-only | grep -cE 'assessment/data|\\.env|\\.key' || true)건\"",
        ):
            self.assertPasses(bash(cmd), cwd=self.bare, msg=cmd)

    def test_echo_의_문구는_경로가_아니다(self):
        self.assertPasses(
            bash('echo "=== .env 류 ==="; ls -a . | grep -i env | head'), cwd=self.bare
        )
        self.assertPasses(
    ...
    def test_작은따옴표_속_마크다운_백틱은_치환이_아니다(self):
        ...
        self.assertPasses(
            bash(
                "sed -i 's/· `.credentials*`/· 경로 모양의 `credentials` "
                "(`~\\/.aws\\/credentials`)/' docs/guard.md"
            ),
            cwd=self.bare,
        )
        self.assertPasses(bash("sed -i 's/old/see `.env.local` docs/' README.md"), cwd=self.bare)
        self.assertBlocked(
            bash('echo "`cat .env.local`"'), cwd=self.bare, msg="큰따옴표 속 백틱은 치환이다"
        )

    def test_큰따옴표_속_문구에_공백이_있으면_경로가_아니다(self):
        self.assertPasses(
            bash('gh pr create --title x --body "see .env.example and docs"'), cwd=self.bare
        )
    ...
    def test_예시를_비밀값_파일로_복사하는_것은_보여주지_않는다(self):
        ...
        self.assertBlocked(
            bash("cp .env.local .env.backup"), cwd=self.bare, msg="원본이 비밀값이면 막는다"
        )
...
class CodeAndProseBodies(GuardCase):
    ...
    def test_python_heredoc_의_key_속성은_코드다(self):
        for body in (
            ...
            self.assertPasses(
                bash(f"uv run python - <<'PY' 2>&1 | tail -22\n{body}\nPY"), cwd=self.bare, msg=body
            )
    ...
    def test_문서에_env_를_적는_heredoc_은_통과한다(self):
        ...
        self.assertPasses(
            ...
            cwd=self.bare,
            msg="heredoc 뒤에 이어지는 명령도 정상 판정",
        )
        self.assertPasses(
            bash("cat > /tmp/pr-body.md <<'MD'\n## 갈래별 판정\n| `.env` 처리 | ✅ |\nMD"),
            cwd=self.bare,
        )
...
class PassThrough(GuardCase):
    ...
    def test_설정_타입이_틀려도_죽지_않는다(self):
        for cfg in (
            '{"protected_paths":["a/"],"allow_substrings":null}',
