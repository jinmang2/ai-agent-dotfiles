...
"이 창에 이제 에이전트가 없다" 를 알 수 없고, 창 이름이 `✅🔵repo` 로 굳는다.
...
| 주인 | 언제 | 무엇을 쓰나 |
|---|---|---|
| 에이전트 | 창을 점유한 동안 | 훅 → `agent/window-label.sh` (이모지 + 마커) |
| 셸 | 프롬프트가 그려질 때 | `PROMPT_COMMAND` → `_cc_shell_window_name` (이름만) |

에이전트가 도는 동안엔 프롬프트가 안 그려지므로 둘이 안 싸운다. 종료되면 셸이
프롬프트를 그리면서 이모지를 뗀다. 덤으로 **이모지 유무 = 에이전트 생존 여부** 라는
의미가 생긴다.
