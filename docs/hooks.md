...
에이전트가 도구 안에서 `claude -p` 를 띄우면 그 중첩 세션도 완전한 세션이라 같은
훅들을 전부 부른다 (창 이름 · 가드 · gstack). 창 이름 훅은 payload 의 `cwd` 가 `/tmp`
류 아래면 이벤트를 통째로 무시한다 — 안 그러면 임시 디렉토리 이름이 창 이름이 되고,
중첩 실행의 `Stop` 이 바깥 세션이 아직 도는데 ✓ 를 찍는다 (`agent/window-label.sh`).
...
`agent/guard.py` (`~/.local/bin/agent-guard`) 는 `PreToolUse` 에서 비밀값 파일 노출과 보호 경로
쓰기를 종료코드 2 로 막는다. 창 이름 훅과 달리 **동기**이고 결과가 도구 실행을 통제한다.
규칙·프로젝트 설정(`.claude/guard.json`)·잔여 위험은 `docs/guard.md`.
