...
| `agent/usage.py` | 심링크 | `~/.local/bin/agent-usage` |
...
```
1:myrepo          기호 없음 = 셸.  cd 하면 따라간다
2:~/dl
3:»myrepo         기호 있음 = 그 창에 에이전트가 살아있다.  창 번호 배경색 = 프로필
4:?client-api

» 작업중(노랑)  ? 입력대기(빨강)  ✓ 완료(초록)   /   번호색: 파랑 claude · 보라 claude-team · 주황 codex
```

창 이름 자체는 라벨뿐이다. 상태 기호와 프로필 색은 훅이 창 옵션(`@cc_sym`,
`@cc_color`)으로 남기고 상태줄 포맷(`tmux/tmux.conf`)이 그린다. 예전엔 이름에
이모지 2개(4칸)를 박았는데, 좁은 상태줄에서 이모지가 먼저 살아남고 정작 라벨이
3자로 잘렸다. 기호 1칸 + 번호 배경색(0칸)으로 바꾸면서 같은 자리에 라벨이 6자
들어간다.
...
후에도 완료 표시가 그대로 굳었다. 지금은 셸 프롬프트가 그려지는 순간 상태 옵션이
지워진다 — 그래서 **상태줄 기호 유무가 에이전트 생존 여부를 뜻하게 됐다.**
...
| `python3 scripts/test-usage.py` | 집계·단가·중복제거 테스트 |
